...
/// MAJOR-B FIX: tombstone TTL 必须远大于 verify cache TTL，防止 cache 残留绕过 revoke
...
/// OPT-1 FIX: 后台 GC，批量清理过期内存 revoke 条目（5 分钟一次）
...
pub async fn set_verify_cache(pool: &RedisPool, key_hash: &str, entry: &VerifyCacheEntry) {
    ...
    if entry.activation_ts <= 0 || entry.expires_at <= 0 {
        tracing::warn!(
            "[Cache] Rejected zero-valued entry for {}...",
            &key_hash[..8.min(key_hash.len())]
        );
        return;
    }
    // OPT-2 FIX: activation_ts >= expires_at 的完整校验
...
/// CRIT-3 FIX: tombstone TTL = max(verify_ttl*100, 86400)，远超任何残留 cache
