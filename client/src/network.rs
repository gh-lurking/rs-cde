...
//
// [BUG-CRIT-3 FIX] parse_server_error 410 分支缺少 return：
//   原代码在 grace==0 时计算 ERR_EXPIRED.to_string() 但未返回，
//   导致 fall through 到通用错误处理。
//   修复：添加 return 关键字。
//   对应 CLAUDE.md §1「Think Before Coding」：仔细审查控制流。
//
...
/// [BUG-HIGH-2 FIX] [BUG-CRIT-3 FIX] 解析服务端错误响应
///
/// 对 410 GONE 响应额外提取 expires_at 和 expiration_grace_secs，
/// 使调用方能判断是否处于宽限期。
///
/// [BUG-CRIT-3 FIX] grace==0 时添加 return，修复 fall-through 缺陷。
fn parse_server_error(status: reqwest::StatusCode, body: &serde_json::Value) -> String {
    ...
    match status.as_u16() {
        410 => {
            ...
            // [BUG-CRIT-3 FIX] 添加 return，修复原代码中 ERR_EXPIRED.to_string()
            // 计算后未返回导致 fall through 到通用处理的缺陷。
            // 对应 CLAUDE.md §1：仔细审查所有控制流路径。
            return ERR_EXPIRED.to_string();
        }
        409 => {
...
pub async fn verify_online(hkey: &str, server_url: &str) -> Result<VerifyResponse, String> {
    ...
    match &result {
        Err(e) if e.starts_with("ERR-TIME-RECORD:server_time=") => {
            ...
            {
                tracing::warn!("[Network] unreasonable RTT or server_ts, abandoning clock retry");
                return result;
            }
            ...
            match &retry {
                Err(e2) if e2.starts_with("ERR-TIME-RECORD:server_time=") => {
                    ...
                    {
                        tracing::warn!("[Network] second retry server_ts2 unreasonable, giving up");
        ...
        // [BUG-HIGH-2 FIX] 过期但含宽限期信息的错误 → 统一视为硬过期
        Err(e) if e.starts_with("ERR-EXPIRED:expires_at=") => Err(ERR_EXPIRED.to_string()),
        _ => result,
    }
}
...
pub async fn validate_system_time() -> Result<(), String> {
    ...
    if local_now < min_ts || local_now > max_ts {
        return Err(format!("system time {} out of reasonable range", local_now));
    }
    ...
    if avg_offset.abs() > SYSTEM_TIME_HARD_LIMIT_SECS {
        return Err(format!(
            "system time offset {}s exceeds limit {}s, sync clock",
    ...
    if avg_offset.abs() > SYSTEM_TIME_WARN_SECS {
        tracing::warn!(
            "[Time] time offset {}s > {}s warning threshold",
            avg_offset,
