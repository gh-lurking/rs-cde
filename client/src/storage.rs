...
//
// ✅ OPT-3 FIX: LocalReadResult::Success 新增 repair_failed: bool 字段
//   当少数副本修复失败时，license_guard.rs 可据此拒绝离线降级
// ✅ 保留原有: MAJOR-B FIX（多数投票算法，N=2: 2:0 通过 / 1:1 Tampered）
...
/// - N=0,1      → Insufficient
/// - N=2, B=2   → Success  ✓
/// - N=2, B=1   → Tampered ✓（无多数）
/// - N=3, B=3   → Success  ✓
/// - N=3, B=2   → Success  ✓（自动修复少数副本，repair_failed 表示修复结果）
