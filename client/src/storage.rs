...
//
// [V4] 增加 MAX_LICENSE_PERIOD 常量用于数据合法性校验
...
fn derive_path(hkey: &str, salt: &str, slot: u8) -> PathBuf {
    ...
    let fname = format!("rs-cde-r{}.dat", &hex[..8]);
    ...
    base.join("rs-cde").join("replicas").join(fname)
}
...
pub fn read_local_record(hkey: &str, salt: &str) -> LocalReadResult {
    ...
    let repair_failed = if best.1 < read_count {
        // 存在不一致：尝试修复少数副本
        match write_all_replicas_return_err(hkey, salt, best.0 .0, best.0 .1) {
            Ok(()) => false,
            Err(_) => true,
        }
    } else {
